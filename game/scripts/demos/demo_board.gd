...
func _draw_turn_rpg(area: Rect2) -> void:
	var party_x: float = area.position.x + area.size.x * 0.24
	var enemy_x: float = area.position.x + area.size.x * 0.74
	var base_y: float = area.position.y + area.size.y * 0.44

	for index in range(3):
		var y: float = base_y + (index - 1) * 58
	...
	var queue_rect: Rect2 = Rect2(area.position + Vector2(34, area.size.y - 82), Vector2(area.size.x - 68, 38))
	draw_rect(queue_rect, Color(0.14, 0.16, 0.18), true)
	draw_rect(queue_rect, Color(0.33, 0.38, 0.40), false, 1.5)
	for index in range(6):
		var x: float = queue_rect.position.x + 28 + index * 56
...
func _draw_arpg(area: Rect2) -> void:
	...
	for index in range(14):
		...
		var radius: float = 115.0 + float(index % 3) * 26.0
		var pos: Vector2 = center + Vector2(cos(angle), sin(angle)) * radius
		draw_circle(pos, 13, Color(0.78, 0.32, 0.24))

	for index in range(5):
		var angle: float = -0.6 + index * 0.28
		var start: Vector2 = center + Vector2(cos(angle), sin(angle)) * 42.0
		var end: Vector2 = center + Vector2(cos(angle), sin(angle)) * 150.0
		draw_line(start, end, Color(0.91, 0.82, 0.42), 3.0)


func _draw_tactics(area: Rect2) -> void:
	var cell: float = min(area.size.x / 10.0, area.size.y / 7.0)
	var origin: Vector2 = area.position + Vector2((area.size.x - cell * 8.0) * 0.5, (area.size.y - cell * 6.0) * 0.5)

	for y in range(6):
		for x in range(8):
			var rect: Rect2 = Rect2(origin + Vector2(x, y) * cell, Vector2(cell - 2, cell - 2))
			var color: Color = Color(0.13, 0.16, 0.17)
...
func _draw_systems_lab(area: Rect2) -> void:
	var lane_y: float = area.position.y + area.size.y * 0.33
	draw_line(Vector2(area.position.x + 42, lane_y), Vector2(area.end.x - 42, lane_y), Color(0.39, 0.36, 0.30), 14.0)
	for index in range(4):
		var tower_pos: Vector2 = Vector2(area.position.x + 120 + index * 100, lane_y + 72)
	...
	var graph_origin: Vector2 = Vector2(area.position.x + 72, area.position.y + area.size.y * 0.72)
	var last: Vector2 = graph_origin
	for index in range(1, 8):
		var next: Vector2 = graph_origin + Vector2(index * 58, -pow(index, 1.45) * 7.0)
...
func _draw_arrow(start_pos: Vector2, end_pos: Vector2, color: Color) -> void:
	...
	var left: Vector2 = direction.rotated(2.55)
	var right: Vector2 = direction.rotated(-2.55)
