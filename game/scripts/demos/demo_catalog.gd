class_name DemoCatalog

static func get_categories() -> Array:
	return [
		{
			"id": "turn_rpg",
			"title": "Turn RPG",
			"summary": "Playable command battle with party roles, deterministic turn order, HP/MP, status ticks, and a readable battle log.",
			"scene_path": "res://scenes/demos/turn_rpg_demo.tscn",
			"loop": [
				"Choose command",
				"Resolve skill and status effects",
				"Advance turn queue",
				"Present battle log",
			],
			"systems": [
				"Unit stats",
				"Skill effects",
				"Status lifecycle",
				"Deterministic RNG",
			],
			"release_checks": [
				"Battle screen loads",
				"Demo data is bundled",
				"One attack resolves consistently",
			],
		},
		{
			"id": "arpg",
			"title": "ARPG / Survivor",
			"summary": "Real-time movement, enemy pressure, cooldown skills, pickups, and short-session combat pacing.",
			"loop": [
				"Move and kite",
				"Auto-fire or cast skills",
				"Collect rewards",
				"Scale waves over time",
			],
			"systems": [
				"Runtime spawning",
				"Cooldown ticking",
				"Area queries",
				"Performance counters",
			],
			"release_checks": [
				"Input responds after export",
				"Frame pacing is acceptable",
				"Timed wave script runs",
			],
		},
		{
			"id": "tactics",
			"title": "Tactics",
			"summary": "Grid movement, range preview, turn ownership, terrain rules, and tactical command resolution.",
			"loop": [
				"Select unit",
				"Preview move and attack range",
				"Commit action",
				"End side turn",
			],
			"systems": [
				"Grid coordinates",
				"Pathfinding",
				"Line and area targeting",
				"Turn ownership",
			],
			"release_checks": [
				"Grid renders cleanly",
				"Selection state survives scene reload",
				"Path preview has no missing assets",
			],
		},
		{
			"id": "systems_lab",
			"title": "Systems Lab / UI Forge",
			"summary": "A shared bucket for tower defense, idle/incremental, cards, roguelite rewards, meta progression, and UI generation tools.",
			"scene_path": "res://scenes/demos/ui_forge_demo.tscn",
			"loop": [
				"Choose a style",
				"Choose or drag a template",
				"Apply generated UI art",
				"Save the customized template",
			],
			"systems": [
				"AI prompt packs",
				"Template JSON",
				"Slot-based skins",
				"One-click preview generation",
			],
			"release_checks": [
				"UI Forge loads",
				"Template save path is writable",
				"Sample skin assets are bundled",
			],
		},
	]
